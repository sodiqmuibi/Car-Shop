    ...
    <Transition as={Fragment} appear show={isOpen}>
        <Dialog as='div' className='relative z-10' onClose={closeModal}>
            ...
            <div className='inset-0 overflow-y-auto fixed'>
                <div className='items-center justify-center p-4 flex min-h-full text-center'>
                    <Transition.Child 
                      as={Fragment} enter='ease-out duration-300'
                      ...
                      leaveFrom='opacity-100 scale-100' leaveTo='opacity-0 scale-95'>
                        <Dialog.Panel className='relative w-full max-h-[90vh] overflow-y-auto transform rounded-2xl transition-all flex flex-col gap-5 bg-white text-left max-w-lg p-6 shadow-xl'>
                            <button type='button' onClick={closeModal} className='absolute top-2 right-2 z-10 bg-white p-2 w-fit rounded-full'>
                                <Image src='/close.svg' alt='close' width={20} height={20} className='object-contain'/>
                            </button>
                            <div className='flex-1 flex flex-col gap-3'>
                                <div className='relative bg-cover w-full h-40 bg-center rounded-lg bg-pattern'>
                                ...
                                <div className='mt-3 flex flex-col gap-4'>
                                    {Object.entries(car).map(([key, value]) => (
                                        <div className='flex items-center justify-between'>
                                            <h2 className='text-gray capitalize'>{key.split("_").join(" ")}</h2>
                                            <p className='tex-black font-semibold'>{value}</p>
                                        </div>
                                    ))}
                                </div>
                            </div>
